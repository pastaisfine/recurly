...
From the project's file list, select up to 15 relevant files as available as an initial selection that might have interesting business value for event tracking, especially conversion and churn events. Also look for additional files related to login that could be used for identifying users, along with error handling. Read the files. If a file is already well-covered by PostHog events, replace it with another option. Do not spawn subagents.

After reviewing the shortlist, treat it as an initial selection, not a coverage boundary: scan all relevant server-side API routes and Server Actions — including payment, webhook, and authentication handlers — and include any omitted files required for complete event tracking.
